...
<div class="m-block-header" id="div-header">
    ...
    <a href="addres" class="m-index-icon">添加</a>
</div>
<div class="addr-wrapp">
    ...
    <div class="addr-list">
        <ul>
            ...
            <li>
                <p>{{$v->province}}{{$v->address_detail}}</p>
            </li>
            <li class="a-set">
                @if($v->status==2)
                <s  class="z-set" style="margin-top: 6px;" gid="{{$v->address_id}}"></s>
                @else
                    <s  class="z-defalt" style="margin-top: 6px;" gid="{{$v->address_id}}"></s>
                @endif
                <span>设为默认</span>
                <div class="fr">
                    <span class="edit" ><a href="editads?address_id={{$v->address_id}}">编辑</a></span>
                    <span class="remove" gid="{{$v->address_id}}">删除</span>
                </div>
            </li>
        </ul>
    </div>
